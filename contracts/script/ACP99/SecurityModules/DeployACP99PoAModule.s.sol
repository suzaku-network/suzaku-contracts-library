...
contract DeployACP99PoAModule is Script {
    function run() external returns (ACP99Manager, ACP99PoAModule) {
        ...
        (uint256 deployerKey, bytes32 subnetID) = helperConfig
            .activeNetworkConfig();

        // Precompute the addresses of ACP99Manager and ACP99PoAModule
        address deployerAddress = vm.addr(deployerKey);
        uint256 deployerNonce = vm.getNonce(deployerAddress);
        address predictedManagerAddress = vm.computeCreateAddress(
            deployerAddress,
            deployerNonce
        );
        address predictedPoAModuleAddress = vm.computeCreateAddress(
            deployerAddress,
            deployerNonce + 1
        );
        ...
        ACP99Manager manager = new ACP99Manager(
            subnetID,
            predictedPoAModuleAddress
        );
        ...
        require(
            address(manager) == predictedManagerAddress,
            "Manager address mismatch"
        );
        require(
            address(poaModule) == predictedPoAModuleAddress,
            "PoAModule address mismatch"
        );
